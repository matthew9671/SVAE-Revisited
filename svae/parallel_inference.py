...
GaussianParams = Dict[str, Array]  # String maps to array


def _make_associative_filtering_elements(params: Dict,
										 potentials: GaussianParams,
										 inputs: Array) -> Tuple:
	...
	def _first_filtering_element(mu: Array,
								 Sigma: Array,
								 m1: Array) -> Tuple[Array, Array, Array, Array, Array]:
	...
	def _generic_filtering_element(mu: Array,
								   Sigma: Array,
								   u: Array) -> Tuple[Array, Array, Array, Array, Array]:
...
def lgssm_filter(params: Dict,
	...
	@vmap
	def filtering_operator(elem1: Tuple[Array, Array, Array, Array, Array],
						   elem2: Tuple[Array, Array, Array, Array, Array]) -> Tuple[Array, Array, Array, Array, Array]:
...
def _make_associative_smoothing_elements(params: Dict,
										 key: PRNGKey,
										 filtered_means: Array,
										 filtered_covariances: Array,
										 inputs: Array,
										 sample_shape: Sequence = ()) -> Tuple:
	...
	def _last_smoothing_element(key: PRNGKey,
								m: Array,
								P: Array) -> Tuple[Array, Array, Array, Array]:
		h = MVN(m, P).sample(seed=key, sample_shape=sample_shape)
		return np.zeros_like(P), m, P, h

	def _generic_smoothing_element(key: PRNGKey,
								   m: Array,
								   P: Array,
								   u: Array) -> Tuple[Array, Array, Array, Array]:
...
def real_lgssm_smoother(params: Dict,
				        emissions: GaussianParams,
				        inputs: Array,
				        key: PRNGKey = None,
				        sample_shape: Sequence = ()) -> Dict:
	...
	@vmap
	def smoothing_operator(elem1: Tuple[Array, Array, Array, Array],
						   elem2: Tuple[Array, Array, Array, Array]) -> Tuple[Array, Array, Array, Array]:
		E1, g1, L1, h1 = elem1
		E2, g2, L2, h2 = elem2
...
def lgssm_log_normalizer(dynamics_params: Dict,
						 mu_filtered: Array,
						 Sigma_filtered: Array,
						 potentials: Dict,
						 inputs: Array) -> float:
	p = dynamics_params
